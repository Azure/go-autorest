...
func NewServicePrincipalToken(id string, secret string, tenentId string) (*Token, error) {
	...
	if response.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("autorest: Token request failed with status code %v", response.StatusCode)
	}

	token := &Token{}
	err = (json.NewDecoder(response.Body)).Decode(token)
	if err != nil {
		return nil, fmt.Errorf("autorest: Failed to deserialize Token (%v)", err)
	}
